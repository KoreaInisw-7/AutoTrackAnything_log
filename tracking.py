...
from config import (DEVICE, INFERENCE_SIZE, IOU_THRESHOLD,
...
if __name__ == '__main__':
    ...
    df = pd.DataFrame(
        columns=['frame_id', 'object_id', 'x1', 'y1', 'x2', 'y2'])
    ...
    with torch.cuda.amp.autocast(enabled=True):
        ...
        while cap.isOpened():
            ...
            print(f"Processing frame {current_frame_index}")
            ...
            if objects_in_video:
                current_features = tracker.get_features_from_clip(frame, yolo_filtered_bboxes)
                current_ids = tracker.match_objects(current_features, yolo_filtered_bboxes)

                if len(class_label_mapping) == 0:  # First objects in video
                    mask = tracker.create_mask_from_img(frame, yolo_filtered_bboxes)
                    ...
                    mask = tracker.create_mask_from_img(frame, filtered_bboxes)
                    unique_labels = np.unique(mask)
                ...
                if current_frame_index % args.yolo_every == 0:
                    filtered_bboxes = get_iou_filtered_yolo_mask_bboxes(yolo_filtered_bboxes, mask_bboxes_with_idx, iou_threshold=args.iou_thresh)

                # VISUALIZATION
                if args.output_video_path is not None:
                    if len(mask_bboxes_with_idx) > 0:
                        for bbox, obj_id in zip(mask_bboxes_with_idx, current_ids):
                            cv2.rectangle(frame, (int(bbox[1]), int(bbox[2])), (int(bbox[3]), int(bbox[4])), (255, 255, 0), 2)
                            cv2.putText(frame, f'{obj_id}', (int(bbox[1])-10, int(bbox[2])-10), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
                ...
                if len(mask_bboxes_with_idx) > 0:
                    for bbox, obj_id in zip(mask_bboxes_with_idx, current_ids):
                        ...
                        x2 = bbox[3]
                        y2 = bbox[4]
                        df.loc[len(df.index)] = [int(current_frame_index), obj_id, x1, y1, x2, y2]
                else:
                    df.loc[len(df.index)] = [int(current_frame_index), None, None, None, None, None]
